...
class CNN(object):
    ...
    def evalute_model(self,learn_rate,max_iter_num,mini_batch_size):
        ...
        for epoch in range(max_iter_num):
            ...
            # 每100次输出一次测试准确率
            if epoch%100 == 0:
                train_accuray = accuracy.eval(feed_dict = {self.x:batch[0],self.y_:batch[1],self.keep_prob:1.0})
                print("epoch:{0},training accuracy: {1}".format(epoch,train_accuray))
            train_step.run(feed_dict={self.x:batch[0],self.y_:batch[1],self.keep_prob:0.5})
        # 计算全部测试集的准确率
        total_accuracy = accuracy.eval(feed_dict = {self.x:self.mnist.test.images,self.y_:self.mnist.test.labels,self.keep_prob:1.0})
